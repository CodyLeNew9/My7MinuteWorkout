...
//Creating the database for Inserting the date of completion of the 7 minute workout.)
// START
/**
...
class SqliteOpenHelper(context: Context, factory: SQLiteDatabase.CursorFactory?) : SQLiteOpenHelper(context, DATABASE_NAME, factory, DATABASE_VERSION) {
    ...
    override fun onCreate(db: SQLiteDatabase) {
        ...
        val CREATE_HISTORY_TABLE = ("CREATE TABLE $TABLE_HISTORY($COLUMN_ID INTEGER PRIMARY KEY,$COLUMN_COMPLETED_DATE TEXT)") // Create History Table Query.
    ...
    //function where the passed Date will be inserted into the Database Table.)
    /**
    ...
    fun getAllCompletedDatesList(): ArrayList<String> {
        ...
        while (cursor.moveToNext()) {
            ...
            val currentColumnIndex = cursor.getColumnIndex(COLUMN_COMPLETED_DATE)
            val currentColumnString = cursor.getString(currentColumnIndex)

            val currentIdIndex = cursor.getColumnIndex(COLUMN_ID)
            val currentIdString = cursor.getString(currentIdIndex)

            Log.i("SQL_READ", "Item $currentIdString is $currentColumnString")

            list.add(currentColumnString) // value is added in the list
